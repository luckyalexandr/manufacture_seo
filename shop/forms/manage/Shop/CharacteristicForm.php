...
class CharacteristicForm extends Model
{
    ...
    public function __construct(Characteristic $characteristic = null, $config = [])
    {
        if ($characteristic) {
            ...
            $this->name_uk = $characteristic->name_uk;
            $this->type = $characteristic->type;
            $this->required = $characteristic->required;
            $this->default = $characteristic->default;
            $this->default_uk = $characteristic->default_uk;
            $this->textVariants = implode(PHP_EOL, $characteristic->variants);
            $this->textVariantsUk = implode(PHP_EOL, $characteristic->variants_uk);
    ...
    public function rules(): array
    {
        return [
            ...
            [['textVariants', 'textVariantsUk'], 'string'],
            [['sort'], 'integer'],
    ...
    {
        return preg_split('#\s+#i', $this->textVariantsUk);
    }
}
